(function(){
  ...
  var color = d3.scale.quantize()
      ...
      .range(d3.range(MAX_ACCIDENTS).map(function(d) { return "q" + d + "-13"; }));
  ...
  d3.csv("../data/part_91_csv/data_1980_84.csv", function(error, csv) {
    var data = d3.nest()
      ...
      //   d[0].fatalities = parseInt(d[0].fatalities); return (!isNaN(d[0].fatalities)) ? d[0].fatalities : 0; })
      // .map(csv);
    ...
    rect.filter(function(d) { return d in data; })
        .attr("class", function(d) {console.log(data[d]); return "day " + color(data[d]); })
      .select("title")
        .text(function(d) {
          if(data[d] == 0)
